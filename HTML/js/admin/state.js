...
class AdminState {
  constructor() {
    this.data = {
      // Properti ini sekarang akan menjadi cerminan dari IndexedDB
      products: [],
      receivables: [],
      payables: [],
      vendors: [],
      // Properti state sementara tetap di sini
      barangMasukList: [],
      purchaseOrderList: [],
    };
    this.user = null;
    this.isOffline = !navigator.onLine; // Tambahkan state offline
      ...
      data = data.filter((item) => {
        ...
        if (dataType === "inventaris") {
          ...
          searchableContent = `${item.invoice_id || ""} ${
            item.no_nota_vendor || ""
          } ${item.outlet_name || ""} ${item.nama_vendor || ""}`.toLowerCase();
        }
  ...
  getSummaryData() {
    ...
    const summary = {
      ...
      produkHabis: inventaris.filter((p) => Number(p.sisa_stok) <= 0).length,
